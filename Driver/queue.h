...
void DeleteJob(struct Queue* Q){
	if(Q->head == Q->tail){
		...
		unsigned int oldHeadPriority = Q->head->jobPriority;
		struct PCB* nextHead = Q->head->next_job;
		...
		//adjust metadata
		if(oldHeadPriority == Q->highestPriority){
