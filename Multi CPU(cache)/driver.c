...
//helper modules
const unsigned int* CacheToRAM(struct CPU* CPU);
...
void CPU(struct Queue* readyQ){
	...
	//jobStart = clock();
  	...
  	do{
  		...
  		for(int count = 0; count != NUM_OF_CPUS; ++count){
  			if(*next_CPU != NULL)
  				(*next_CPU)->CPU_registers.i_register = Fetch((*next_CPU)->program_counter);
  		...
  		for(int count = 0; count != NUM_OF_CPUS; ++count){
  			if(*next_CPU != NULL){
  				if((*next_CPU)->program_counter == (*next_CPU)->current_job_end_cache){
  					//COPY CACHE
  					CacheToRAM((*next_CPU));
  					
  					//SCAN CACHE
  					//printf("Job #%u finished!\n", ((*next_CPU)->currentJobID));
  					//for(int i = 0; i != (*next_CPU)->currentJobTotalSize; ++i)
  					//	printf("%u\n", (*next_CPU)->cache[i]);
...
const unsigned int* CacheToRAM(struct CPU* CPU){
	//write to RAM from cache
  	unsigned int* RAM_cursor = CPU->current_job_start_RAM;
  	unsigned int* cache_cursor = CPU->current_job_start_cache;
  	while(cache_cursor != (CPU->current_job_start_cache + CPU->currentJobTotalSize)){
 		*RAM_cursor = *cache_cursor;
 		++RAM_cursor;
 		++cache_cursor;
  	}
}
...
void Dispatcher(struct CPU* CPU_ptr, const struct PCB* job){
  	...
  	CPU_ptr->currentJobTotalSize = job->jobTotalSize;
  	CPU_ptr->current_job_start_RAM = job->program_start;
  	CPU_ptr->current_job_end_RAM = (job->program_start + (job->jobTotalSize - 1));
  	CPU_ptr->current_job_start_cache = CPU_ptr->cache;
  	CPU_ptr->program_counter = CPU_ptr->cache;
  	
  	//write to cache
  	unsigned int* RAM_cursor = CPU_ptr->current_job_start_RAM;
  	unsigned int* cache_cursor = CPU_ptr->cache;
  	while(RAM_cursor != (CPU_ptr->current_job_end_RAM)){
 		*cache_cursor = *RAM_cursor;
 		++RAM_cursor;
 		++cache_cursor;
  	}
  	CPU_ptr->current_job_end_cache = CPU_ptr->current_job_start_cache + (CPU_ptr->currentJobTotalSize - 1);
}
...
const unsigned int* Scheduler(struct Queue* newQ, struct Queue* readyQ){
	...
	while((readyQ->size != 4) && (newQ->size != 0)){
